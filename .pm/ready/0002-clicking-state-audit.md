...
2. The "click flicker" bug is the longstanding one and is the heart of this card. Symptom: after the JamBox window regains focus from another window/app, the first single-click on a row visually selects the intended row for a frame and then snaps back to the previously-selected row. The user has not given a deterministic repro — treat it as intermittent, find one if you can.
