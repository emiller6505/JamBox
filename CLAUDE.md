...
Automated tests live in `JamBoxTests/` and run via `xcodebuild test`. The suite uses XCTest. Layer 1 static checks (`JamBoxTests/StaticChecks/`) scan `JamBox/*.swift` as plain text to enforce regression-defense invariants — most importantly that every `AVURLAsset(` construction includes `AVURLAssetPreferPreciseDurationAndTimingKey`. Audio fixtures live in `JamBoxTests/Fixtures/` and are regenerated by `JamBoxTests/Fixtures/generate.sh`. See `TESTING.md` at the repo root for framework rationale, what each Layer 1 check defends against, and how to add fixtures and tests.
